...
const UserManagement: React.FC = () => {
  ...
  // Função formatRole (linha ~174):
const formatRole = (role: string): string => {
  if (!role) return 'Usuário'; // ← ADICIONE ESTA LINHA
...
// Função formatSector (linha ~185):
const formatSector = (sector: string): string => {
  if (!sector) return 'Não definido'; // ← ADICIONE ESTA LINHA
                  ...
                  {filteredUsers.length > 0 ? (
                    filteredUsers.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.role === 'super_admin'
